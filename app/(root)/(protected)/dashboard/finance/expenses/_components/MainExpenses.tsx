...
export default function MainExpenses({
  ...
  // ✅ moneda por defecto: la del user (si existe), sino la primera, sino USD
  ...
  // ✅ si cambia defaultCurrency por load inicial/props, sincroniza
  useEffect(() => {
  ...
  const primaryCurrencyCodeSafe =
    currencies.find((c) => c.code === primaryCurrencyCode)?.code || defaultCurrency;
  ...
  const formatTotalsLine = (totals: Record<string, number>) => {
    ...
    const ordered = [
      ...codes.filter((c) => c === primaryCurrencyCodeSafe),
      ...codes.filter((c) => c !== primaryCurrencyCodeSafe),
    ];

    return ordered.map((code) => formatMoneyByCode(code, totals[code], currencyMap[code])).join(' • ');
  };
                      ...
                      return (
                        ...
                        >
                          <div className="flex h-10 w-10 items-center justify-center rounded-lg border bg-background">
                            {isImg ? (
                              <img src={a.url} alt={a.fileName || 'soporte'} className="h-10 w-10 rounded-lg object-cover" />
                            ) : isPdf ? (
