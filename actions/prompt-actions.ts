...
/**
 * **1. Crear (Create)**
 * Crea un nuevo registro de PromptInstancia.
 * @param formData - Datos del formulario.
 */
export async function createPromptInstancia(formData: FormData): Promise<ActionResponse<PromptInstancia>> {
  ...
  try {
    const newPrompt = await db.promptInstancia.create({
      data: {
        ...validation.data,
      },
    });
 ...
 */
export async function getPromptsByUserId(userId: string): Promise<ActionResponse<PromptInstancia[]>> {
  ...
  try {
    const prompts = await db.promptInstancia.findMany({
      where: { userId },
      orderBy: { id: "desc" },
    });
    return {
 ...
 * @param formData - Datos del formulario.
 */
export async function updatePromptInstancia(id: number, formData: FormData): Promise<ActionResponse<PromptInstancia>> {
  ...
  try {
    const updatedPrompt = await db.promptInstancia.update({
      where: { id },
...
export async function deletePromptInstancia(id: number): Promise<ActionResponse<void>> {
  ...
  try {
    await db.promptInstancia.delete({
      where: { id },
    });
    revalidatePath('/dashboard/prompts');
    return {
