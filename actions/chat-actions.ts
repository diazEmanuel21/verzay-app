...
export async function sendMediaByUrl(
    ...
    try {
        ...
        if (!response.ok) {
            ...
            const apiMsg = (Array.isArray(errorResponse) ? errorResponse.join(', ') : (errorResponse as string) || (raw?.message as string)) || `Error ${response.status} al enviar media.`;
            return { success: false, message: apiMsg, raw, remoteJid };
        }
