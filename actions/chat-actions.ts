...
export async function fetchChatsFromEvolution(
  ...
  if (!baseUrlRaw || !key || !instanceName) return { success: false, message: 'Parámetros de conexión faltantes.' };
  ...
  try {
    ...
    if (!res.ok && (res.status === 404 || res.status === 405)) res = await doRequest(endpoint, key, ctrl.signal, 'GET');
    clearTimeout(t);
    const raw = await res.json().catch(() => null);
    if (!res.ok) return { success: false, message: (raw?.message as string) || `Error ${res.status} en la API.` };
    if (!raw) return { success: false, message: 'Respuesta inválida o vacía.' };
...
export async function findMessagesByRemoteJid(
  ...
  instanceName: string,
  remoteJid: string,
  options?: { timeoutMs?: number; page?: number; pageSize?: number; limit?: number }
